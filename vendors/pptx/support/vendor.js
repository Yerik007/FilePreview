...
import tXml from "@/components/FliesView/vendors/pptx/lib/tXml";
import d3 from "d3";
import nv from "nvd3";
import dingbat_unicode from "../lib/dingbat";
...
export async function getContentTypes(zip) {
...
export async function getSlideSizeAndSetDefaultTextStyle(zip) {
    //get app version
...
export async function processSingleSlide(zip, sldFileName, index, slideSize) {
            ...
            switch (element["attrs"]["Type"]) {
                ...
                case "http://schemas.microsoft.com/office/2007/relationships/diagramDrawing":
                    ...
                    slideResObj[element["attrs"]["Id"]] = {
                        type: element["attrs"]["Type"].replace(
    ...
    const slideMasterResFilename =
        masterFilename.replace("slideMasters/slideMaster", "slideMasters/_rels/slideMaster") +
        ".rels";
...
async function processPicNode(node, warpObj, source, sType) {
    ...
    if ((vdoNode === undefined && audioNode === undefined) || !mediaProcess || !mediaSupportFlag) {
        ...
        if (audioNode !== undefined) {
            rtrnData +=
                '<audio id="audio_player" controls ><source src="' + audioBlob + '"></audio>';
    ...
    for (var i = 0; i < apNode.length; i++) {
        ...
        var sld_prg_width =
            prg_width_node !== undefined
                ? "width:" + computedFactor(parseInt(prg_width_node)) + "px;"
                : "width:inherit;";
        var sld_prg_height =
            prg_height_node !== undefined
                ? "height:" + computedFactor(parseInt(prg_height_node)) + "px;"
                : "";
        ...
        text +=
            ...
            cssName +
            "' >";
        var buText_ary = await genBuChar(
            pNode,
            i,
            spNode,
            textBodyNode,
        ...
        var isBullate =
            buText_ary[0] !== undefined && buText_ary[0] !== null && buText_ary[0] != ""
                ? true
                : false;
        var bu_width =
        ...
        if (rNode === undefined && pNode !== undefined) {
            // without r
            var prgr_text = await genSpanElement(
                pNode,
                undefined,
                spNode,
                textBodyNode,
...
async function genBuChar(node, i, spNode, textBodyNode, pFontStyle, idx, type, warpObj) {
    ...
    if (rNode !== undefined) {
        dfltBultColor = await getFontColorPr(
            rNode,
            spNode,
            lstStyle,
            pFontStyle,
...
async function genSpanElement(
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    //getFontColor
    var fontClrPr = await getFontColorPr(
        node,
        pNode,
        lstStyle,
        pFontStyle,
            ...
            if (tcNodes.constructor === Array) {
                ...
                while (j < tcNodes.length) {
                    if (rowSpanAry[j] == 0 && totalColSpan == 0) {
                        ...
                        var cellParmAry = await getTableCellParams(
                            tcNodes[j],
                            getColsGrid,
                            i,
                            j,
                ...
                if (rowSpan !== undefined) {
                    ...
                    tableHtml +=
                        "<td class='" + cssName + "' style='" + colStyl + "'>" + text + "</td>";
                }
            }
        }
        tableHtml += "</tr>";
...
function extractChartData(serNode) {
    ...
    if (serNode["c:xVal"] !== undefined) {
        ...
        eachElement(serNode, function (innerNode, index) {
            var dataRow = new Array();
            var colName =
                getTextByPathList(innerNode, ["c:tx", "c:strRef", "c:strCache", "c:pt", "c:v"]) ||
                index;

            // Category (string or number)
            var rowNames = {};
            if (
                getTextByPathList(innerNode, ["c:cat", "c:strRef", "c:strCache", "c:pt"]) !==
                undefined
            ) {
                eachElement(
                ...
                getTextByPathList(innerNode, ["c:cat", "c:numRef", "c:numCache", "c:pt"]) !==
                undefined
            ) {
                eachElement(
            ...
            // Value
            if (
                getTextByPathList(innerNode, ["c:val", "c:numRef", "c:numCache", "c:pt"]) !==
                undefined
            ) {
                eachElement(
...
function applyShade(rgbStr, shadeValue, isAlpha) {
    ...
    if (isAlpha) return tinycolor({ h: color.h, s: color.s, l: cacl_l, a: color.a }).toHex8();
...
function applyLumOff(rgbStr, offset, isAlpha) {
    ...
    if (lum >= 1) {
        if (isAlpha) return tinycolor({ h: color.h, s: color.s, l: 1, a: color.a }).toHex8();
        return tinycolor({ h: color.h, s: color.s, l: 1, a: color.a }).toHex();
    }
    if (isAlpha) return tinycolor({ h: color.h, s: color.s, l: lum, a: color.a }).toHex8();
...
function applyHueMod(rgbStr, multiplier, isAlpha) {
    ...
    if (isAlpha) return tinycolor({ h: cocacl_h, s: color.s, l: color.l, a: color.a }).toHex8();
...
function applySatMod(rgbStr, multiplier, isAlpha) {
    ...
    if (isAlpha) return tinycolor({ h: color.h, s: cacl_s, l: color.l, a: color.a }).toHex8();
...
function processSingleMsg(d) {
    ...
    if (chart !== null) {
        d3.select(`#${chartID}`).append("svg").datum(data).transition().duration(500).call(chart);
