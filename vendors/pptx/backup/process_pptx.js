...
export default function processPptx(setOnMessage = () => {}, postMessage) {
    ...
    setOnMessage(async e => {
        switch (e.type) {
            case "processPPTX": {
                try {
                    await processPPTX(e.data, e.options);
                } catch (e) {
    ...
    async function processPPTX(data, settings) {
    ...
    async function processSingleSlide(zip, sldFileName, index, slideSize) {
            ...
            for (const element of RelationshipArray) {
                switch (element["attrs"]["Type"]) {
                    case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                        themeFilename = element["attrs"]["Target"].replace("../", "ppt/");
                        break;
                    default:
                        masterResObj[element["attrs"]["Id"]] = {
                            type: element["attrs"]["Type"].replace(
                                "http://schemas.openxmlformats.org/officeDocument/2006/relationships/",
                                ""
                            ),
        ...
        for (let nodeKey in nodes) {
            if (nodes[nodeKey].constructor === Array) {
                for (const element of nodes[nodeKey]) {
                    result += await processNodesInSlide(nodeKey, element, warpObj);
                }
            } else {
        ...
        for (let key in spTreeNode) {
            ...
            if (targetNode.constructor === Array) {
                for (const element of targetNode) {
                    const nvSpPrNode = element["p:nvSpPr"];
                    ...
                    if (idx !== undefined) {
                        idxTable[idx] = element;
                    }
                    if (type !== undefined) {
                        typeTable[type] = element;
                    }
                }
            } else {
    ...
    async function processNodesInSlide(nodeKey, nodeValue, warpObj, source, sType) {
        let result = "";
    ...
    async function processGroupSpNode(node, warpObj) {
        ...
        for (let nodeKey in node) {
            if (node[nodeKey].constructor === Array) {
                for (const element of node[nodeKey]) {
                    result += await processNodesInSlide(nodeKey, element, warpObj);
                }
            } else {
...
*/
    ...
    function genGlobalCSS() {
        ...
        for (let key in styleTable) {
            cssText +=
                "section ." + styleTable[key]["name"] + "{" + styleTable[key]["text"] + "}\n";
        }
        return cssText;
    }
        ...
        if (trNodes.constructor === Array) {
            for (let i = 0; i < trNodes.length; i++) {
                ...
                if (i === 0 && firstRowAttr !== undefined) {
                    ...
                    rowsStyl +=
                        " background-color:#" + fillColor + ";" + " opacity:" + colorOpacity + ";";
...
*/
        ...
        if (bgPr !== undefined) {
            ...
            if (bgFillTyp === "SOLID_FILL") {
                ...
                for (let i = 0; i < gsLst.length; i++) {
                    if (i === gsLst.length - 1) {
                        bgcolor +=
                            "rgba(" + hexToRgbNew(colorArray[i]) + "," + tintArray[i] + ")" + ");";
                    } else {
                        bgcolor +=
                            "rgba(" + hexToRgbNew(colorArray[i]) + "," + tintArray[i] + ")" + ", ";
                    }
            ...
            if (bgPr !== undefined) {
                ...
                if (bgFillTyp === "SOLID_FILL") {
                    ...
                    // a:stretch => a:fillRect =>attrs (l:-17000, r:-17000)
                    bgcolor =
                        "background-image: url(" + picFillBase64 + ");  z-index: " + ordr + ";";
                    // console.log(warpObj)
                }
    ...
    function getGradientFill(node) {
        ...
        for (let i = 0; i < gsLst.length; i++) {
            ...
            if (gsLst[i]["a:srgbClr"] !== undefined) {
                let lumMod =
                    parseInt(getTextByPathList(node, ["a:srgbClr", "a:lumMod", "attrs", "val"])) /
                    100000;
                let lumOff =
                    parseInt(getTextByPathList(node, ["a:srgbClr", "a:lumOff", "attrs", "val"])) /
                    100000;
                if (isNaN(lumMod)) {
                    lumMod = 1.0;
                }
                ...
                let lumMod =
                    parseInt(
                        getTextByPathList(gsLst[i], ["a:schemeClr", "a:lumMod", "attrs", "val"])
                    ) / 100000;
                let lumOff =
                    parseInt(
                        getTextByPathList(gsLst[i], ["a:schemeClr", "a:lumOff", "attrs", "val"])
                    ) / 100000;
                if (isNaN(lumMod)) {
                    lumMod = 1.0;
                }
    ...
    function getSchemeColorFromTheme(schemeClr, sldMasterNode) {
        ...
        if (slideLayoutClrOvride === "" || slideLayoutClrOvride === undefined) {
            slideLayoutClrOvride =
                getTextByPathList(sldMasterNode, ["p:sldMaster", "p:clrMap", "attrs"]) || {};
        }
        // console.log(slideLayoutClrOvride);
